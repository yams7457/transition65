...
function noFlatNine()
    ...
    for _, pentadValue in ipairs(pentads[currentPentad]) do
        for _, bassValue in ipairs(bassCollectionNoDuplicates) do
            if bassValue == pentadValue - 1 or bassValue == pentadValue - 6 or bassValue == pentadValue + 6 then
...
function adjustAndPlayThePivot(direction, x, y)
    ...
    record_grid_value(noteCoordinates[lastMelodyIndex][1], noteCoordinates[lastMelodyIndex][2], 1)
...
function g.key(x, y, z, dup)
  ...
  if x == 12 or x == 16 then
    if y <= 6 and z == 0 then
      record_grid_value(noteCoordinates[lastMelodyIndex][1], noteCoordinates[lastMelodyIndex][2], 0)
    end
  end
  
  if y == 6 and x == 11 and z == 1 then
  ...
  elseif y == 7 and x == 11 and z == 1 then
  ...
  elseif y == 8 and x == 11 and z == 1 then
    grid_pattern:set_overdub((grid_pattern.overdub + 1) % 2) -- toggles overdub
  end
    
    if y <= 6 and x >= 13 and x <= 15 and dup ~= 1 then
    ...
    if z == 1 then
        ...
        if y >= 7 and x >= 12 then
            bassNote(x,y)
        end
        if y == 3 and x == 11 then
            compArm = 1
        end
        if y == 2 and x == 11 then
            bassArm = 1
        end
        if x <= 4 then
...
function gridmap()
    g:all(0)
    if grid_pattern.rec == 1 then 
	    g:led(11, 6, 15)
    else
	    g:led(11, 6, 5)
    end

    if grid_pattern.play == 1 then 
	    g:led(11,7,15)
    else
	    g:led(11,7,5)
    end

    if grid_pattern.overdub == 1 then 
	    g:led(11,8,15)
    else
	    g:led(11,8,5)
    end
    ...
    g:led(11, 1, math.min(math.ceil(15 * bassStep / params:get('bassEvery')), 15))
    ...
    if params:get("evolveProb") == 0 then
        g:led(4, 8, 15)
    else g:led(4, math.floor(((100 - params:get("evolveProb")) / 14.3 ) + 1), 15)
    end
    g:refresh()
  end
...
function changeProbabilities(x,y)
    ...
    elseif x == 4 then
        params:set("evolveProb", math.floor(100 - (y - 1) * 14.3))
    end
end
...
function parse_grid_pattern(data)
  if data.z == 1 then 
    if data.scale ~= currentPentad then
      currentPentad = data.scale
    end
    if math.random(1,100) <= params:get("evolveProb") then
    
      local changedScaleIndex = data.index
    
      local eventNumber = findIndex(data, grid_pattern.event)
    
      local correspondingOffEventIndex = nil
    
    
      for i = #grid_pattern.event, eventNumber, -1 do
          if grid_pattern.event[i]["index"] == changedScaleIndex and grid_pattern.event[i]["z"] == 0 then
            correspondingOffEventIndex = i
          end
      end

      data.index = math.random((math.max(8, data.index - 4)),math.min(25, data.index + 4))

      data.x = noteCoordinates[data.index][1]
      data.y = noteCoordinates[data.index][2]
    
      grid_pattern.event[correspondingOffEventIndex]["x"] = noteCoordinates[data.index][1]
      grid_pattern.event[correspondingOffEventIndex]["y"] = noteCoordinates[data.index][2]
      grid_pattern.event[correspondingOffEventIndex]["index"] = data.index
